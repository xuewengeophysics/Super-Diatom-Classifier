...
# %%
img = cv2.imread("./tmp/AOVA.png",cv2.IMREAD_UNCHANGED)
taxon = split[0]
print(taxon)
height, width = img.shape[0], img.shape[1]
img_gs = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
# Finding common parts
sub = img-img_comp
# ret,img_thr = cv2.threshold(sub,(0,0,0),255,cv2.THRESH_BINARY)
sub_thr = cv2.inRange(sub, (0,0,0),(1,1,1))
kernel_size = 2
kernel = np.ones((kernel_size,kernel_size),np.uint8)
mask = cv2.morphologyEx(sub_thr, cv2.MORPH_CLOSE, kernel)
...
for cnt in contours:
    x, y, w, h = cv2.boundingRect(cnt)
    if w*h > height*width*0.005 and w*h<height*width*0.95 and w>10 and h>10:
        ROI = img_gs[y-padding:y+h+padding,x-padding:x+w+padding]
        showImg(ROI)
        # saveImg(ROI, "tmp0", taxon, j)
        j+=1
